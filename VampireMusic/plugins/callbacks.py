...
from VampireMusic import app, VampireMusic, db, lang, queue, tg, yt
from VampireMusic.helpers import admin_check, buttons, can_manage_vc
...
async def _controls(_, query: types.CallbackQuery):
    ...
    if action == "pause":
        ...
        await VampireMusic.pause(chat_id)
        if qaction:
        ...
        await VampireMusic.resume(chat_id)
        if qaction:
            return await query.edit_message_reply_markup(
        ...
        await VampireMusic.play_next(chat_id, skip_user=user)
        try:
            return await query.answer()
        except Exception:
            return
        ...
        return await VampireMusic.play_media(chat_id, msg, media)

    elif action == "replay":
        media = queue.get_current(chat_id)
        media.user = user
        await VampireMusic.replay(chat_id)
        status = query.lang["replayed"]
        reply = query.lang["play_replayed"].format(user)

    elif action == "stop":
        await VampireMusic.stop(chat_id)
        ...
        await VampireMusic.play_media(chat_id, query.message, media, new_pos)
        media.time = new_pos
    ...
    elif action in ["more", "cthumb", "cautoplay", "back"]:
        # Answer the query first
        if action in ["cthumb", "cautoplay"]:
            await query.answer()
